// Dual architecture source. The AVR branch preserves the supplied AVR source.
// Select exactly ONE.
...
#endif
#if !defined(AVR_ARCH) && !defined(X64_ARCH)
#error "Define AVR_ARCH or X64_ARCH"
#endif
...
void deriveTLSKeys()
{
    ...
    //printHex(        F("SERVER_WRITE_KEY="),        serverWriteKey,        16    );
...
uint8_t tlsReadServerHandshake()
{
    ...
    tlsPrfSha256(tlsMasterSecret, 48, label, 15, serverHash, 32, tlsHmacInnerHash, 12);
    // Compare against decrypted verify_data (bytes 4..15)
    for (uint8_t i = 0; i < 12; i++) {
        if (ciphertext[4 + i] != tlsHmacInnerHash[i]) {
        ...
        while (client.connected())
        {
            ...
            if (contentType == 0x16)
            {
                ...
                else if (firstByte == 0x0E)
                {
                    ...
                    {
                        tlsHmacInnerHash[4 + i] =
                            tlsHmacInnerHash[i];
                    }
                    // Serial.print(F("CONNECTED BEFORE FINISHED COMPUTE: "));
                    // Serial.println(client.connected());
                    tlsHmacInnerHash[0] = 0x14;
                    tlsHmacInnerHash[1] = 0x00;
                    tlsHmacInnerHash[2] = 0x00;
                    tlsHmacInnerHash[3] = 0x0C;
                    for (uint8_t i = 0; i < 16; i++)
                    {
                        tlsTranscriptUpdateByte(tlsHmacInnerHash[i]);
                    }
...
public:
    ...
    void print(uint8_t v) { std::printf("%u", (unsigned)v); }
    void print(int8_t v) { std::printf("%d", (int)v); }
    void print(uint16_t v) { std::printf("%u", (unsigned)v); }
    void print(uint32_t v) { std::printf("%u", (unsigned)v); }
    void print(uint64_t v) { std::printf("%llu", (unsigned long long)v); }
    void print(bool v) { std::fputs(v ? "1" : "0", stdout); }
    void print(uint8_t v, int base) { if (base == HEX) std::printf("%02X", (unsigned)v); else print(v); }
    ...
    bool connect(const char *host, uint16_t port)
    {
        ...
        for (addrinfo *rp = result; rp; rp = rp->ai_next)
        {
            ...
            if (::connect(s, rp->ai_addr, rp->ai_addrlen) == 0)
            {
                ...
                std::printf("TCP SOCKET CONNECTED\n");
                return true;
            }

            std::printf("connect: errno=%d (%s)\n",
                        errno,
                        std::strerror(errno));
...
static void gcmCtrCrypt(const uint8_t key[16], uint8_t counter[16],uint8_t *data,  uint16_t length)
{
    ...
    while (length)
    {
        ...
        std::printf("GCM_CTR=");
        for (uint8_t i = 0; i < 16; i++)
            std::printf("%02X", counter[i]);
        std::printf("\n");

        std::printf("GCM_KS=");
        for (uint8_t i = 0; i < 16; i++)
            std::printf("%02X", stream[i]);
        std::printf("\n");
        // Serial.print(F("KS="));
        // for (uint8_t i = 0; i < 16; i++)
        // {
    ...
    const uint8_t *ciphertext,    uint16_t ciphertextLen,    uint8_t tag[16])
{
    ...
    if (aadLen)
    {
        ...
        std::printf("GHASH_AFTER_AAD=");
        for (uint8_t i = 0; i < 16; i++)
            std::printf("%02X", ctx.Y[i]);
        std::printf("\n");
    }
...
static bool tlsSendGCMRecord(    uint8_t contentType,    uint8_t *plaintext,    uint16_t plaintextLength)
{
    ...
    std::printf("GCM_KEY=");
for (uint8_t i = 0; i < 16; i++)
    std::printf("%02X", clientWriteKey[i]);
std::printf("\n");
    ...
    std::printf("GCM_KEY_ACTUAL=");
    for (uint8_t i = 0; i < 16; i++)
        std::printf("%02X", clientWriteKey[i]);
    std::printf("\n");

    std::printf("GCM_COUNTER_ACTUAL=");
    for (uint8_t i = 0; i < 16; i++)
        std::printf("%02X", counter[i]);
    std::printf("\n");
    // Serial.print(F("SRAM AFTER CTR: "));
    ...
    std::printf("WIRE_RECORD=");
    for (uint8_t i = 0; i < plaintextLength; i++)
        std::printf("%02X", plaintext[i]);
    for (uint8_t i = 0; i < 16; i++)
        std::printf("%02X", counter[i]);
    std::printf("\n");
...
void deriveTLSKeys()
{
    ...
    std::printf("ECDHE_BYTES=");
    for (int i = 0; i < 32; i++)
        std::printf("%02X", ecdheSharedSecret.v[i]);
    std::printf("\n");
    // ============================================================
    // MASTER SECRET SEED
    // client_random || server_random
    ...
    for (uint8_t i = 0; i < 16; i++)
    {
        ...
        std::printf("AFTER_REVERSE=");
    for (int i = 0; i < 32; i++)
        std::printf("%02X", ecdheSharedSecret.v[i]);
    std::printf("\n");


    std::printf("ECDHE_PRF=");
for (int i = 0; i < 32; i++)
    std::printf("%02X", ecdheSharedSecret.v[i]);
std::printf("\n");
    ...
    const uint8_t masterLabel[] PROGMEM =
        "master secret";
    std::printf("CLIENT_RANDOM=");
    for (uint8_t i = 0; i < 32; i++)
        std::printf("%02X", clientRandom[i]);
    std::printf("\n");

    std::printf("SERVER_RANDOM=");
    for (uint8_t i = 0; i < 32; i++)
        std::printf("%02X", serverRandom[i]);
    std::printf("\n");
    tlsPrfSha256(
        ecdheSharedSecret.v,
        32,
    ...
    std::printf("MASTER_SECRET=");
    for (uint8_t i = 0; i < 48; i++)
        std::printf("%02X", tlsMasterSecret[i]);
    std::printf("\n");
    ...
    std::printf("KEY_BLOCK=");
    for (uint8_t i = 0; i < 32; i++)
        std::printf("%02X", ecc.temp3.v[i]);
    for (uint8_t i = 0; i < 8; i++)
        std::printf("%02X", ecc.temp4.v[i]);
    std::printf("\n");
    // ============================================================
    // CLIENT WRITE KEY
    // ============================================================
    ...
    std::printf("CLIENT_KEY_AFTER_COPY=");
    for (uint8_t i = 0; i < 16; i++)
        std::printf("%02X", clientWriteKey[i]);
    std::printf("\n");
    ...
    std::printf("SERVER_WRITE_KEY=");
    for (uint8_t i = 0; i < 16; i++)
        std::printf("%02X", serverWriteKey[i]);
    std::printf("\n");
    ...
    serverWriteIV[i] =
        ecc.temp4.v[4 + i];
    std::printf("SERVER_WRITE_IV=");
    for (uint8_t i = 0; i < 4; i++)
        std::printf("%02X", serverWriteIV[i]);
    std::printf("\n");
    ...
    uint16_t outputLength)
{
    ...
    std::printf("OUTPUT1=%p OUTPUT2=%p\n",
            (void*)output1,
            (void*)output2);
    // printHex(F("PRF_SECRET="), secret, secretLength);
    // printHex(F("PRF_LABEL="), label, labelLength);
    ...
    for (uint8_t i = 0; i < seedLength; i++)
        tlsPrfInput[labelLength + i] = ecc.product[i];
    std::printf("PRF_A1_INPUT=");
    for (uint8_t i = 0; i < labelSeedLength; i++)
        std::printf("%02X", tlsPrfInput[i]);
    std::printf("\n");
    ...
    while (produced < outputLength)
    {
        ...
        std::printf("PRF_BLOCK=");
        for (uint8_t i = 0; i < 32; i++)
            std::printf("%02X", prfBlock[i]);
        std::printf("\n");
        ...
        std::printf("PRF_NEXT_A=");
        for (uint8_t i = 0; i < 32; i++)
            std::printf("%02X", tlsPrfA[i]);
        std::printf("\n");
...
uint8_t tlsReadServerHandshake()
{
    ...
    std::printf("SERVER_EXPLICIT_NONCE=");
    for (uint8_t i = 0; i < 8; i++)
        std::printf("%02X", explicitNonce[i]);
    std::printf("\n");
    ...
    std::printf("SERVER_RECEIVED_TAG=");
    for (uint8_t i = 0; i < 16; i++)
        std::printf("%02X", receivedTag[i]);
    std::printf("\n");
    //Serial.println(F("ALL 16 TAG BYTES RECEIVED"));
    // --- VERIFY GCM TAG (Server Finished) ---
    // Build 12-byte nonce = serverWriteIV (4 bytes) || explicitNonce (8 bytes)
    ...
    gcmIncrementCounter(counter);  // now counter = J0 + 1
    gcmCtrCrypt(serverWriteKey, counter, ciphertext, 16); // decrypt in place
    printf("SERVER_FINISHED_PLAINTEXT=");
    for (uint8_t i = 0; i < 16; i++)
    ...
    tlsTranscriptFinal(serverHash);
    printf("SERVER_FINISHED_TRANSCRIPT=");
    for (uint8_t i = 0; i < 32; i++)
    ...
    printf("SERVER_EXPECTED_FINISHED=");
    for (uint8_t i = 0; i < 12; i++)
        printf("%02X", serverFinished[i]);
    printf("\n");
...
uint8_t runTLS()
{
    ...
    while (millis() - start < 15000UL)
    {
        ...
        while (client.connected())
        {
            ...
            std::printf(
                "RX RECORD type=%02X ver=%02X%02X len=%u\n",
                contentType,
                versionMajor,
                versionMinor,
                recordLength
            ...
            if (contentType == 0x16)
            {
                ...
                if (firstByte == 0x02)
                {
                    ...
                    for (uint8_t i = 0; i < 32; i++)
                        std::printf("%02X", debugTranscript[i]);
                    std::printf("\n");
                }
                ...
                else if (firstByte == 0x0C)
                {
                    ...
                    std::printf("SERVER_PUB_X=");
                    for (int8_t i = 31; i >= 0; i--)
                        std::printf("%02X", ecc.point1.x.v[i]);
                    std::printf("\n");

                    std::printf("SERVER_PUB_Y=");
                    for (int8_t i = 31; i >= 0; i--)
                        std::printf("%02X", ecc.point1.y.v[i]);
                    std::printf("\n");
                    ...
                    std::printf("ECDHE_SHARED=");
                    for (int8_t i = 31; i >= 0; i--)
                        std::printf("%02X", ecc.temp1.v[i]);
                    std::printf("\n");
                    //Serial.println("A");
                    // ------------------------------------------------
                    // TLS KEYS
                ...
                else if (firstByte == 0x0E)
                {
                    ...
                    printf("MASTER_SECRET_CHECK=");
                    for (uint8_t i = 0; i < 48; i++)
                        printf("%02X", tlsMasterSecret[i]);
                    printf("\n");
                    std::printf("CLIENT_FINISHED_TRANSCRIPT=");
                    for (uint8_t i = 0; i < 32; i++)
                        std::printf("%02X", clientFinishedTranscript[i]);
                    std::printf("\n");
                    // ------------------------------------------------
                    // CLIENT FINISHED VERIFY DATA
                    // ------------------------------------------------
                    ...
                    std::printf("FINISHED BYTES=");
                    for (uint8_t i = 0; i < 16; i++)
                        std::printf("%02X", clientFinished[i]);
                    std::printf("\n");
                    // Serial.print(F("CONNECTED AFTER FINISHED COMPUTE: "));
                    // Serial.println(client.connected());
                    // ------------------------------------------------
                    ...
                    std::printf("AFTER_CLIENT_FINISHED=");
                    for (uint8_t i = 0; i < 32; i++)
                        std::printf("%02X", afterClientFinished[i]);
                    std::printf("\n");
                   ...
                   std::printf("FINISHED BYTES=");
                        for (uint8_t i = 0; i < 16; i++)
                            std::printf("%02X", tlsHmacInnerHash[i]);
                        std::printf("\n");
                        ...
                        std::printf("CLIENT_KEY_FINAL=");
                        for (uint8_t i = 0; i < 16; i++)
                            std::printf("%02X", clientWriteKey[i]);
                        std::printf("\n");

                        std::printf("CLIENT_IV_FINAL=");
                        for (uint8_t i = 0; i < 4; i++)
                            std::printf("%02X", clientWriteIV[i]);
                        std::printf("\n");
                        std::printf("MASTER_SECRET=");
                        for (uint8_t i = 0; i < 48; i++)
                            std::printf("%02X", tlsMasterSecret[i]);
                        ...
                        std::printf("KEY_BLOCK=");
                        for (uint8_t i = 0; i < 48; i++)
                            std::printf("%02X", tlsHmacKeyBlock[i]);
                        std::printf("\n");
                        if (!tlsSendGCMRecord(
                                0x16,
                                clientFinished,
